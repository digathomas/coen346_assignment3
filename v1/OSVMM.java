    ...
    public static void main(String[] args) throws FileNotFoundException, InterruptedException {
        ...
        while (commandsFile.hasNextLine()) {
            ...
            if (command[0].equals("Store")) { // if the command is store
                ...
                System.out.println("Command not available."); // if there is a function that is not Store, Lookup or
                                                              // Release
            }
        ...
        // Read processesFile
        VMM virtualMemory = new VMM(memoryArray, clock, fwriter);
        while (processesFile.hasNextLine()) { // read the porcesses file in the same way
            ...
            Process pro = new Process(arrivalTime, burstTime, processNumber, commandList, virtualMemory, CPUAccess,
                    clock);
            processes.add(pro); // add the process to the list

        }
        Scheduler schedule = new Scheduler(processes, ready, running, terminated, clock, numberOfProcesses, CPUAccess,
                fwriter); // create scheduler
