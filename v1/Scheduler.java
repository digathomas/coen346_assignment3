...
public class Scheduler extends Thread {
    ...
    AtomicInteger CPUAccess = new AtomicInteger(2); // CPU access keys , limited to two since only 2 processes can run
                                                    // at the same time
    FileWriter fwriter=null;
    int numberOfProcesses; // number of total processes

    Scheduler(List<Process> processList, List<Process> readyList, List<Process> runningList,
            List<Process> terminatedList, Clock clock1, int numOfPro, AtomicInteger key, FileWriter fw) {
    ...
    public static void move(List<Process> departure, List<Process> destination) { // move the process from departure to
                                                                                  // destination
    ...
    public void run() {
        ...
        while (terminated.size() < numberOfProcesses) // if the size of the terminated list is smaller than that of the
                                                      // number of processes
        {
            ...
            if (processes.isEmpty())
                flag = 1;
            while (flag == 0 && CPUAccess.get() > 0) { // check if there are enough keys to enter the while loop, so it
                                                       // can add a process

                if (processes.get(0).getArrival() == clock.getTime()) { // if you check the process list and there is a
                                                                        // process with proper arrival time
                    ...
                    if (processes.isEmpty())
                        flag = 1; // if processes is empty, then flag is set to 1, and while loop condition is no
                                  // longer satisfied
                } else
                    flag = 1;
            }
            ...
            while (CPUAccess.get() > 0 && flag2 == 0) {
                ...
                if (!ready.isEmpty()) { // while the ready list is not empty get a process from there
                    ...
                    try {
                        fwriter.write("Clock: " + clock.getTime() + " Process: " + running.get(0).getProcessID()
                                + " Resumed");
                    } catch (IOException e) {
                    ...
                    if(!running.isEmpty()&& running.get(0).getArrival()==clock.getTime())
						try {
							fwriter.write("Clock: "+ clock.getTime()+" Process: " + running.get(0).getProcessID()+ " Started");
						} catch (IOException e) {
                 ...
                 //System.out.println(CPUAccess.get());
                    ...
                    clock.increment(); //increment the clock by the time quantum
                         ...
                         if(process1 !=null)
                             try {
                                 fwriter.write("Clock: " + clock.getTime() + " Process: " + process1.getProcessID()
                                         + " Paused");
                             } catch (IOException e) {
                         ...
                         if(!running.isEmpty())
                             try {
                                 fwriter.write("Clock: " + clock.getTime() + " Process: "
                                         + running.get(0).getProcessID() + " Paused");
                             } catch (IOException e) {
                                 e.printStackTrace();
                             }
